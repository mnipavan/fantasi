...
vScaleFac = 0.0001
xShift = -0.00001
drdt=Expression("aFac*( ((kParm - 1.0)/(x[0] - xshift)) - ((kParm / pow(lambParm, kParm))*pow((x[0]-xshift), (kParm - 1.0))))",aFac = vScaleFac, kParm=kParam, lambParm=lambParam, xshift= xShift, degree=3)

#### Create VTK file for saving solution
vtkfile = File('WeibullTransient/solution.pvd')
...
sgma_val = 0.05
aFac = 0.01
rho_D=Expression('aFac * exp(-0.5 * (x[0] - 0.5) * (x[0] - 0.5) / (sig * sig))', aFac = aFac, sig = sgma_val, dnfac = 2.0*np.pi,degree = q_degree)
...
rho_D=Expression('shift + aFac * exp(-0.5 * (x[0] - 0.5) * (x[0] - 0.5) / (sig * sig))', shift=err_init, aFac = aFac, sig = sgma_val, dnfac = 2.0*np.pi,degree = q_degree)
rho_curr=interpolate(rho_D,V)
